...
## 1. Philosophy: 工具主权 (Tool Sovereignty)

在 Module 1 接近尾声时，我们需要建立一个最高维度的认知：**授权即主权，核验即自由。**

### 1.1 从代码主权到工具主权
在 Module 1 的前半部分（L1-L5），我们通过手敲代码和手动运行，建立了初步的“代码主权”。但当我们在 L6 跨入 IDE，尤其是现在进入 AI 自动化循环时，我们的主权提升到了一个新的层次——**工具主权**。

*   **主权的表现**：以前你是通过“自己做”来保证正确；现在你是通过“授权 AI 做”并“核验 AI 的动作”来保证正确。
*   **核验的跨越**：
    *   **Level 1 (L1-L5)**：核验**自己**写的一行代码或一个命令对不对。
    *   **Level 2 (L9)**：核验 **AI** 帮你生成的一整个功能、一次自动化环境配置、一次复杂的终端执行对不对。

如果你没有前面的基础知识，AI 的自动化对你来说就是一个无法审计的“黑盒”。只有掌握了工具主权，你才有能力看穿 AI 做的每一个动作（它用了什么命令？动了哪个文件？），从而真正成为人机协作中的驾驶员。

### 1.2 主权的最高表现：授权 (Permission)
为什么你依然是“指挥官”？因为在 **Antigravity** 的控制权天平上，它做的每一个敏感动作，都必须经过你的**手点确认 (Approve)**。
*   这便是 AI 时代的主权：你掌握着开启自动化魔法的红色按钮。如果你不点那个绿色勾选按钮，它就动弹不得。
