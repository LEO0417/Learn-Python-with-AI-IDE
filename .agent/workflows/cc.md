...
3.  **Terminology Alignment**: Use `chinese-terminology-checker` to ensure SC uses Mainland China terms and TC uses Taiwan terms.
...
5.  **Title Verification**: Match titles in file headers, intro/outro text, `MAPPING.md`, and task trackers.
6.  **Title Snapshot**: Update `docs/FINALIZED_LESSONS_TITLES.md` if the current lesson is finalized, ensuring the SC/TC/EN titles are correctly reflected in the uniform format.
